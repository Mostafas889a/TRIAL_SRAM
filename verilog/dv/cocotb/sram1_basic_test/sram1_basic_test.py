...
@cocotb.test()
@report_test
async def sram1_basic_test(dut):
    caravelEnv = await test_configure(dut, timeout_cycles=1000000)
    cocotb.log.info("[TEST] Start sram1_basic_test")
    ...
    if pulses == 2:
        cocotb.log.info("[TEST] SRAM1 basic test PASSED")
    else:
        cocotb.log.error("[TEST] SRAM1 basic test FAILED")
        assert False, "SRAM1 test failed"
